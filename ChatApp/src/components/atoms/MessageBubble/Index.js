...
//   });
  ...
  const highlightedMessage = message.split(' ').map((word, index) => {
    const isHighlighted = searchQuery && word.toLowerCase().includes(searchQuery.toLowerCase());
    return (
      <Text key={index} style={[styles.messageText, isHighlighted && styles.highlightedText]}>
        {word}{' '}
      </Text>
    );
  });
     ...
     <Text style={styles.messageText}>
      ...
      {isOutgoing && (
        <View style={styles.timestampContainer}>
          <Text style={styles.timestamp}>{timestamp?.seconds ? new Date(timestamp.seconds * 1000).toLocaleTimeString(): ''}</Text>
          {status === 'sent' ? (
